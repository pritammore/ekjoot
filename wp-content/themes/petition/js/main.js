(function($) {
    ...
    // DIMMER
    $(document).on("hover", '.blogs.post .image, .team, .petition-list-card .ui.image, .petition-card .image', function(e) {
    ...
    // POPUP
    $('#share-message').popup();
    $('.social-share .button, .social-share .item, .supporter-avatar, .contributer-avatar').popup();
    ...
    // DROPDOWN MENU
    $('.edit-update, .more-share, #search_topics, select.dropdown, .give-type-button').dropdown();
    ...
    $('#topics-search').dropdown({
        apiSettings: {
            url: main_vars.home_url + '/topics-search?q={query}'
        },
    ...
    $('#organization-search').dropdown({
        apiSettings: {
            url: main_vars.home_url + '/organization-search?q={query}'
        },
        allowReselection: true,
        allowAdditions: true,
        hideAdditions: false,
    ...
    $(add_button_level).click(function(e) { //on add input button click
        e.preventDefault();
        if (x < max_fields) { //max input box allowed
            x++; //text box increment
    ...
    $('#skip-after-sign').on('click', function() {
        $('#sign-successful-modal').modal('hide')
    })
