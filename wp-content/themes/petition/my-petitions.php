...
<div class="ui container mobile-full">
    ...
    <?php
        $my_petitions = conikal_my_petitions($current_user->ID);
        if($my_petitions) {
            $total_p = $my_petitions->found_posts;
        } else {
            $total_p = 0;
        }
    ?>
    ...
    <?php
        $users = get_users();
        $following = $current_user->follow_user;
        $followers = array();
        foreach ($users as $user) {
            $follow_user = get_user_meta($user->data->ID, 'follow_user', true);
            if(is_array($follow_user) && in_array($current_user->ID, $follow_user)) {
                $user_id = $user->ID;
    ...
    <div class="ui grid mobile-full">
            ...
            <div class="five wide column computer only">
                <div class="ui sticky" id="about-sticky">
                    <div class="ui segment">
                        ...
                        <?php echo ($current_user->description) ? esc_html($current_user->description) : ''; ?>
                        <div class="ui list">
                            <?php if ($current_user->user_country || $current_user->user_state || $current_user->user_city) { ?>
                                <div class="item"><i class="marker icon"></i>
                                    <div class="content">
                                    <?php echo ($current_user->user_city ? esc_html($current_user->user_city . ', ') : '') . ($current_user->user_state ? esc_html($current_user->user_state . ', ') : '') . ($current_user->user_country ? esc_html($current_user->user_country) : '') ?>
                                    </div>
                                </div>
                            <?php } ?>
                            <?php if ($current_user->user_url) { ?>
                                <div class="item"><i class="linkify icon" style="float: left;"></i>
                                    ...
                                    <a target="_blank" href="<?php echo esc_html($current_user->user_url) ?>"><?php echo esc_html($current_user->user_url) ?></a>
                                    </div>
                                </div>
                            <?php } ?>
                            <?php if ($current_user->user_birthday) { ?>
                                <div class="item"><i class="birthday icon"></i>
                                    <div class="content">
                                    <?php echo esc_html('Born on ', 'petition') . esc_html( date('d F Y', strtotime($current_user->user_birthday)) ) ?>
                                    </div>
                                </div>
                            <?php } ?>
                            <div class="item"><i class="calendar outline icon"></i>
                                <div class="content">
                                <?php echo esc_html('Joined ', 'petition') . esc_html( date('d F Y', strtotime($current_user->user_registered)) ) ?>
                                </div>
                            </div>
                        </div>
                        <?php if ($followers) { ?>
                ...
                <div class="ui segment">
                    ...
                    <div class="ui list">
                        ...
                        <?php if ($current_user->user_url) { ?>
                            <div class="item"><i class="linkify icon" style="float: left;"></i>
                                <div class="content truncate">
                                <a target="_blank" href="<?php echo esc_attr($current_user->user_url) ?>"><?php echo esc_html($current_user->user_url) ?></a>
                                </div>
                            </div>
                        <?php } ?>
                        <?php if ($current_user->user_birthday) { ?>
        ...
        <?php if($my_petitions->have_posts()) { ?>
            <div id="content">
                <?php while( $my_petitions->have_posts() ) {
                    $my_petitions->the_post();
                    $id = get_the_ID();
                    $approved = get_post_status($id);
                ...
                <div class="ui segments petition-list-card">
                    <?php if ($approved == 'pending') { ?>
                        <div class="ui active centered inverted dimmer">
                            <div class="content">
                              <div class="center">
                                <h2 class="ui icon header">
                                    <i class="warning icon"></i>
                                    <?php esc_html_e('Pending approval!', 'petition') ?>
                                </h2>
                              </div>
                            </div>
                        </div>
                            ...
                            <div class="sixteen wide mobile six wide tablet six wide computer column">
                                <a class="ui fluid image blurring" href="<?php echo esc_url($link) ?>" target="_blank" data-bjax>
                                    ...
                                    <?php if(has_post_thumbnail()) { ?>
                                        <img class="ui fluid image" id="thumbnail-post" src="<?php echo esc_url(the_post_thumbnail_url('petition-thumbnail')) ?>" alt="<?php echo esc_attr($title) ?>">
                                    <?php } elseif ($gallery) { ?>
