...
/**
 * Function to add petition to bookmarks
 */
if( !function_exists('conikal_add_to_bookmarks') ): 
    function conikal_add_to_bookmarks() {
        check_ajax_referer('bookmark_ajax_nonce', 'security');
        $post_id = isset($_POST['post_id']) ? sanitize_text_field($_POST['post_id']) : '';
        $user_id = isset($_POST['user_id']) ? sanitize_text_field($_POST['user_id']) : '';

        $bookmarks_key = 'petition_bookmark';
        $bookmarks = get_user_meta($user_id, $bookmarks_key, true);
        if($bookmarks == '') {
            $bookmarks = array();
            delete_user_meta($user_id, $bookmarks_key);
            add_user_meta($user_id, $bookmarks_key, $bookmarks);
        }
        if(in_array($post_id, $bookmarks) === false) {
            array_push($bookmarks, $post_id);
            update_user_meta($user_id, $bookmarks_key, $bookmarks);
        }

        echo json_encode(array('status'=>true, 'bookmarks'=>$bookmarks));

        die();
    }
endif;
add_action( 'wp_ajax_nopriv_conikal_add_to_bookmarks', 'conikal_add_to_bookmarks' );
add_action( 'wp_ajax_conikal_add_to_bookmarks', 'conikal_add_to_bookmarks' );

/**
 * Function to remove petition from bookmarks
 */
if( !function_exists('conikal_remove_from_bookmarks') ): 
    function conikal_remove_from_bookmarks() {
        check_ajax_referer('bookmark_ajax_nonce', 'security');
        $post_id = isset($_POST['post_id']) ? sanitize_text_field($_POST['post_id']) : '';
        $user_id = isset($_POST['user_id']) ? sanitize_text_field($_POST['user_id']) : '';
...
add_action( 'wp_ajax_nopriv_conikal_remove_from_bookmarks', 'conikal_remove_from_bookmarks' );
add_action( 'wp_ajax_conikal_remove_from_bookmarks', 'conikal_remove_from_bookmarks' );


/**
 * FUNTION GET PETITIONS IS BOOKMARKED
 */
if( !function_exists('conikal_bookmark_petitions') ): 
    function conikal_bookmark_petitions($user_id) {
        ...
        $bookmarks = get_user_meta($user_id, 'petition_bookmark', true);
...
// LOAD MORE BOOKMARKED PETITIONS

if( !function_exists('conikal_load_bookmark_petitions') ): 
    function conikal_load_bookmark_petitions() {
...
add_action( 'wp_ajax_nopriv_conikal_load_bookmarked_petitions', 'conikal_load_bookmarked_petitions' );
add_action( 'wp_ajax_conikal_load_bookmarked_petitions', 'conikal_load_bookmarked_petitions' );
