<?php
/*
Template Name: Petition Bookmarked
*/
...
$bookmark_petitions = conikal_bookmark_petitions($current_user->ID);
if($bookmark_petitions) {
    $total_p = $bookmark_petitions->found_posts;
} else {
    $total_p = 0;
}
$users = get_users();
...
<div class="ui container mobile mobile-full">
    ...
    <div class="page-content">
        <div class="ui grid mobile-full">
            ...
            <?php if($bookmark_petitions != false) { ?>
                <div id="content">
                    <?php while ( $bookmark_petitions->have_posts() ) {
                        $bookmark_petitions->the_post();
                        $id = get_the_ID();
                ...
                <!-- LOAD MORE BUTTON -->
                <button class="ui basic fluid large button" id="load-more" data-page="2" data-number="<?php echo esc_attr($posts_per_page) ?>" data-author="<?php echo esc_attr($current_user->ID) ?>" data-type="conikal_load_bookmark_petitions"><i class="long arrow down icon"></i><?php echo __('Load more...', 'petition'); ?></button>
                <?php } else {
